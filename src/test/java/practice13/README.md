写一个Person类，要有id，name，age属性，靠id来判断是否是同一个人。要有一个introduce方法， introduce方法返回一个字符串形如：
...
写一个接口，自行命名，我们这里称为接口X，在引入一个类，叫Computer，Computer有一个name属性，Computer不是Person的子类。 Computer和Teacher都实现了这个接口X，Klass不再直接通知Teacher，而是这个接口X的实现类。
